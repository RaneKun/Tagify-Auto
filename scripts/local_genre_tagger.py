...
# Stage 3 of the pipeline: only rescan the songs the ONLINE genre tagger
# couldn't classify (cataloged under failed/no_genre/) — but the real audio
# to reprocess always lives in temporary_output/, the shared working copy.
NO_GENRE_CATALOG_DIR = FAILED_DIR / "no_genre"
NO_GENRE_CATALOG_DIR.mkdir(parents=True, exist_ok=True)
MUSIC_FOLDER = str(NO_GENRE_CATALOG_DIR)   # used in log messages below
...
class _SuppressConsoleNoise(logging.Filter):
    _BLOCK_PHRASES = (
        ...
        "Rescued: removed stale no_genre catalog entry", # rescue log line
    )
...
def move_to_failed(filepath: str, error_type: str) -> None:
    ...
    later stages (lyrics, spotify) can still process it for everything else
    it still needs.
    """
...
def rescue_from_no_genre_catalog(fname: str) -> None:
    """Remove a file's stale entry from failed/no_genre/ now that a genre was
    actually found for it — keeps the catalog accurate."""
    stale = NO_GENRE_CATALOG_DIR / fname
    try:
        if stale.exists():
            stale.unlink()
            logger.info(f"Rescued: removed stale no_genre catalog entry for {fname}")   # filtered from console now
    except Exception as e:
        logger.warning(f"Could not remove stale no_genre catalog entry for {fname}: {e}")
...
def main() -> None:
    ...
    # Scan scope is failed/no_genre/ (what the ONLINE genre tagger couldn't
    # classify) — the actual audio to reprocess lives in temporary_output/.
    all_files = sorted(
        f for f in os.listdir(NO_GENRE_CATALOG_DIR)
        if f.lower().endswith(".ogg")
    )
    total = len(all_files)
    logger.debug(f"Total candidate OGG files in failed/no_genre: {total}")

    done_set, progress = load_progress()

    # Keep only done files that actually exist in the catalog
    ...
    if total == 0:
        banner([
            ...
            f"Nothing to do — failed/no_genre/ is empty.",
            f"Every song already has a genre from the online tagger!",
        ])
        print(f"\n  {c_dim('nothing here for me to do, moving on (｡•̀ᴗ-)✧')}\n")
        logger.debug("No candidate files in failed/no_genre — exiting cleanly.")
        return
    ...
    for i, fname in enumerate(all_files, 1):
        filepath = str(TEMP_OUTPUT_DIR / fname)   # the REAL working copy, not the stale catalog copy
        ...
        if not os.path.isfile(filepath):
            ...
            logger.warning(f"{fname} listed in failed/no_genre but missing from temporary_output — skipping")
            continue
        ...
        if result == "ok":
            ...
            rescue_from_no_genre_catalog(fname)   # a genre WAS found — clear the stale catalog entry
