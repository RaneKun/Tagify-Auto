...
SOURCE_MUSIC_DIR = str(_cfg.get("source_music_directory", "")).strip()
if not SOURCE_MUSIC_DIR:
    sys.exit("[!] config.json is missing \"source_music_directory\" — edit config.json and set it before running.")

# This is stage 1 of the pipeline: read the TRUE source directly (read-only,
# never modified) and write every tagged copy into temporary_output/ instead.
MUSIC_FOLDER    = SOURCE_MUSIC_DIR
...
def move_to_failed(filepath: str, error_type: str) -> None:
    ...
    This COPIES (never moves) — the source file and/or the temporary_output
    working copy are left completely untouched so later stages can still pick
    the file up for anything else it still needs.
    """
...
def process_file(filepath: str, global_stats: dict, debug: bool = False) -> str:
    """
    Copy one OGG file into temporary_output/, analyse THAT copy, and write BPM
    ...
    try:
        ...
        # Don't leave a half-tagged copy sitting in temporary_output
        try:
...
def main() -> None:
    ...
    for i, fname in enumerate(all_files, 1):
        ...
        if result == "ok":
            ...
            # Read back tags for display and stats (from the temporary_output
