...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  CONFIG  ─  Spotify credentials live in ../config.json                   ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
_spotify_cfg = _cfg.get("spotify", {}) if isinstance(_cfg.get("spotify", {}), dict) else {}
SPOTIFY_CLIENT_ID:     str = str(_spotify_cfg.get("client_id", "")).strip()
SPOTIFY_CLIENT_SECRET: str = str(_spotify_cfg.get("client_secret", "")).strip()
if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET:
    sys.exit("[!] config.json is missing spotify.client_id / spotify.client_secret — edit config.json and set them before running.")
...
# Stage 6 (final) of the pipeline: every file currently in temporary_output/
# gets a Spotify pass, then is copied into output/ — the true final folder —
# whether or not a Spotify match was found.
MUSIC_FOLDER = str(TEMP_OUTPUT_DIR)
...
def move_to_failed(filepath: str, error_type: str) -> None:
    ...
    Copies (never moves) — the temporary_output copy is what still gets
    carried into output/ regardless, per the final-pass rule below.
    """
