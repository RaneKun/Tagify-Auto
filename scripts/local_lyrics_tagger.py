...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  CONFIG  ─  fully local/offline: no source dir or API keys needed here,  ║
# ║  so unlike the other scripts this one does not read ../config.json       ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
TEMP_OUTPUT_DIR = PARENT_DIR / "temporary_output"
TEMP_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Stage 5 of the pipeline: double-check BOTH of the online lyrics tagger's
# soft-miss buckets — instrumental (it may be wrong!) and no_lyrics — using
# real audio analysis (Demucs + Whisper) instead of title keywords/LRCLib
# flags. The real audio to re-check always lives in temporary_output/.
INSTRUMENTAL_CATALOG_DIR = FAILED_DIR / "instrumental"
NO_LYRICS_CATALOG_DIR    = FAILED_DIR / "no_lyrics"
INSTRUMENTAL_CATALOG_DIR.mkdir(parents=True, exist_ok=True)
NO_LYRICS_CATALOG_DIR.mkdir(parents=True, exist_ok=True)
MUSIC_FOLDER = str(TEMP_OUTPUT_DIR)   # used in log messages below
...
def move_to_failed(filepath: str, error_type: str) -> None:
    ...
    Copies (never moves) — the temporary_output working copy stays put so
    the spotify_tagger final pass can still process it.
    """
...
def sync_lyrics_catalogs(fname: str, final_status: str) -> None:
    """Keep failed/instrumental and failed/no_lyrics accurate after this
    script's own (more reliable, audio-based) re-check — regardless of which
    of the two catalogs the file originally came from.

    final_status == "ok"           -> lyrics found: clear it from BOTH catalogs
    final_status == "instrumental" -> confirmed instrumental: make sure it's
                                       ONLY cataloged under failed/instrumental
    """
    instrumental_copy = INSTRUMENTAL_CATALOG_DIR / fname
    no_lyrics_copy    = NO_LYRICS_CATALOG_DIR / fname

    if final_status == "ok":
        for stale in (instrumental_copy, no_lyrics_copy):
            try:
                if stale.exists():
                    stale.unlink()
                    logger.info(f"Rescued: removed stale '{stale.parent.name}' catalog entry for {fname}")
            except Exception as e:
                logger.warning(f"Could not remove stale catalog entry {stale}: {e}")
    elif final_status == "instrumental":
        try:
            if no_lyrics_copy.exists():
                no_lyrics_copy.unlink()
                logger.info(f"Reclassified {fname}: no_lyrics → instrumental")
        except Exception as e:
            logger.warning(f"Could not remove stale no_lyrics entry for {fname}: {e}")
...
def main() -> None:
    ...
    # Scan scope is BOTH failed/instrumental/ (online tagger may be wrong!)
    # and failed/no_lyrics/ — the real audio to re-check lives in
    # temporary_output/. Checked BEFORE loading Demucs/Whisper so an empty
    # run doesn't pay for a slow model load.
    candidate_names = set()
    candidate_names.update(f for f in os.listdir(INSTRUMENTAL_CATALOG_DIR) if f.lower().endswith(".ogg"))
    candidate_names.update(f for f in os.listdir(NO_LYRICS_CATALOG_DIR) if f.lower().endswith(".ogg"))
    all_files = sorted(candidate_names)
    total = len(all_files)
    logger.debug(f"Total candidate OGG files in failed/instrumental + failed/no_lyrics: {total}")
    ...
    if total == 0:
        banner([
            ...
            f"Nothing to do — failed/instrumental/ and failed/no_lyrics/ are both empty.",
            f"Every song already has lyrics (or a confirmed instrumental) from the online tagger!",
        ])
        print(f"\n  {c_dim('nothing here for me to do, moving on (｡•̀ᴗ-)✧')}\n")
        logger.debug("No candidate files in failed/instrumental or failed/no_lyrics — exiting cleanly.")
        return

    print(f"{c_dim(f'Found {total} candidate file(s) across failed/instrumental + failed/no_lyrics.')}")
    print()
    ...
    for i, fname in enumerate(all_files, 1):
        filepath = os.path.join(MUSIC_FOLDER, fname)   # MUSIC_FOLDER == temporary_output/
        ...
        if not os.path.isfile(filepath):
            print(f"{prefix} {c_yellow('⚠ skipping —')} {fname} {c_dim('(missing from temporary_output)')}")
            logger.warning(f"{fname} listed in failed/instrumental or failed/no_lyrics but missing from temporary_output — skipping")
            continue
        ...
        if result == "instrumental":
            ...
            sync_lyrics_catalogs(fname, "instrumental")   # in case it came from no_lyrics
            ...
            sync_lyrics_catalogs(fname, "ok")   # rescued — clear it out of both catalogs
            current_done.add(fname)
