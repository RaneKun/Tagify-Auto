...
# Resolve the script's own directory so logs/ lands next to the script's
# parent folder, exactly as every other tagger in this set does it.
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  CONFIG  ─  source dir + API keys live in ../config.json             ║
# ╚══════════════════════════════════════════════════════════════════════════╝

PARENT_DIR = SCRIPT_DIR.parent

# ── Shared config.json — source music dir + API keys ─────────────────────────
CONFIG_FILE = PARENT_DIR / "config.json"
try:
    _cfg = json.loads(CONFIG_FILE.read_text(encoding="utf-8"))
except Exception as exc:
    sys.exit(f"[!] Could not read config.json at {CONFIG_FILE}: {exc}")

FAILED_DIR      = PARENT_DIR / "failed"
TEMP_OUTPUT_DIR = PARENT_DIR / "temporary_output"
TEMP_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Stage 2+ of the pipeline: read/write temporary_output/ — the true source
# directory was only ever touched by bpm_mood_tagger and is never revisited.
MUSIC_DIR: str = str(TEMP_OUTPUT_DIR)
...
_lastfm_cfg = _cfg.get("lastfm", {}) if isinstance(_cfg.get("lastfm", {}), dict) else {}
LASTFM_API_KEY: str = str(_lastfm_cfg.get("api_key", "")).strip()
...
_mb_cfg = _cfg.get("musicbrainz", {}) if isinstance(_cfg.get("musicbrainz", {}), dict) else {}
MUSICBRAINZ_APP_NAME:    str = str(_mb_cfg.get("app_name", "RaneKunGenreTagger"))
MUSICBRAINZ_APP_VERSION: str = str(_mb_cfg.get("app_version", "1.0"))
MUSICBRAINZ_CONTACT:     str = str(_mb_cfg.get("contact", "not-provided@example.com"))
...
def main() -> None:
    ...
    if ENABLE_LASTFM and not LASTFM_API_KEY:
        logger.warning("config.json lastfm.api_key is empty — disabling Last.fm source.")
        print(f"  {c_yellow('[WARN]')} config.json's lastfm.api_key is empty — Last.fm source disabled for this run.")
        ENABLE_LASTFM = False
    ...
    for i, filepath in enumerate(all_ogg, 1):
        ...
        status, detail = process_file(filepath, artist_cache)
